...
from pluto.settings import DATA_SETS, PORTAL_CNA_FILE
...
class TestCopyNumber(PlutoTestCase):
    ...
    def test_run_copy_number_one_file(self):
        ...
        path = expected_output['output_cna_file']['path']
        with open(path) as f:
            header = next(f)
        ...
        path = expected_output['output_cna_ascna_file']['path']
        with open(path) as f:
            header = next(f)
        header_parts = header.split()
        ...
        path = expected_output['output_cna_scna_file']['path']
        with open(path) as f:
            header = next(f)
        header_parts = header.split()
        expected_header_parts = ['Hugo_Symbol', 's_C_VJ7F47_P001_d']
